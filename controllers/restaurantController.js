...
exports.viewResto = async function(req, res){
	try{
		// const user = await userModel.getCurrent(req.params.id2);
		...
		res.render('restaurant', {ppic: resto.images[0], h_user: resto.name, fullname: resto.name, blogs: reviews, loggedIn: false, numReviews: reviews.length});
	}
	catch (error){
		console.log(error);
		res.status(500).send("Internal Server Error")
